...
describe('Xiangqi Move Validation', () => {
	describe('King (General) moves', () => {
		test('should move one square orthogonally within palace', () => {
			...
			expect(isValidMove(board, { row: 9, col: 4 }, { row: 8, col: 4 })).toBe(
				true
			);
			expect(isValidMove(board, { row: 9, col: 4 }, { row: 9, col: 3 })).toBe(
				true
			);
			expect(isValidMove(board, { row: 9, col: 4 }, { row: 9, col: 5 })).toBe(
				true
			);
		});
		...
		test('should not move diagonally', () => {
			...
			expect(isValidMove(board, { row: 9, col: 4 }, { row: 8, col: 3 })).toBe(
				false
			);
		});
		...
		test('should not leave palace', () => {
			...
			expect(isValidMove(board, { row: 9, col: 3 }, { row: 9, col: 2 })).toBe(
				false
			);
		});
	...
	describe('Advisor moves', () => {
		test('should move diagonally within palace', () => {
			...
			expect(isValidMove(board, { row: 9, col: 3 }, { row: 8, col: 4 })).toBe(
				true
			);
		});
		...
		test('should not move orthogonally', () => {
			...
			expect(isValidMove(board, { row: 9, col: 3 }, { row: 8, col: 3 })).toBe(
				false
			);
		});
		...
		test('should not leave palace', () => {
			...
			expect(isValidMove(board, { row: 7, col: 3 }, { row: 6, col: 2 })).toBe(
				false
			);
		});
	});
	...
	describe('Elephant moves', () => {
		test('should move exactly two squares diagonally', () => {
			...
			expect(isValidMove(board, { row: 9, col: 2 }, { row: 7, col: 4 })).toBe(
				true
			);
			expect(isValidMove(board, { row: 9, col: 2 }, { row: 7, col: 0 })).toBe(
				true
			);
		});
		...
		test('should not cross river', () => {
			...
			expect(isValidMove(board, { row: 6, col: 2 }, { row: 4, col: 4 })).toBe(
				false
			);
		});
	});
	...
	describe('Horse moves', () => {
		test('should move in L-shape', () => {
			...
			expect(isValidMove(board, { row: 5, col: 4 }, { row: 3, col: 3 })).toBe(
				true
			);
			expect(isValidMove(board, { row: 5, col: 4 }, { row: 3, col: 5 })).toBe(
				true
			);
			expect(isValidMove(board, { row: 5, col: 4 }, { row: 4, col: 2 })).toBe(
				true
			);
			expect(isValidMove(board, { row: 5, col: 4 }, { row: 4, col: 6 })).toBe(
				true
			);
		});
	...
	describe('Chariot moves', () => {
		test('should move orthogonally any distance', () => {
			...
			expect(isValidMove(board, { row: 5, col: 4 }, { row: 0, col: 4 })).toBe(
				true
			);
			expect(isValidMove(board, { row: 5, col: 4 }, { row: 9, col: 4 })).toBe(
				true
			);
			expect(isValidMove(board, { row: 5, col: 4 }, { row: 5, col: 0 })).toBe(
				true
			);
			expect(isValidMove(board, { row: 5, col: 4 }, { row: 5, col: 8 })).toBe(
				true
			);
		});
		...
		test('should not move diagonally', () => {
			...
			expect(isValidMove(board, { row: 5, col: 4 }, { row: 3, col: 6 })).toBe(
				false
			);
		});
		...
		test('should be blocked by pieces', () => {
			...
			expect(isValidMove(board, { row: 5, col: 4 }, { row: 2, col: 4 })).toBe(
				false
			);
			expect(isValidMove(board, { row: 5, col: 4 }, { row: 3, col: 4 })).toBe(
				true
			); // Can capture
		});
	});
	...
	describe('Cannon moves', () => {
		test('should move orthogonally without capturing', () => {
			...
			expect(isValidMove(board, { row: 5, col: 4 }, { row: 5, col: 7 })).toBe(
				true
			);
		});
		...
		test('should capture by jumping over exactly one piece', () => {
			...
			expect(isValidMove(board, { row: 5, col: 4 }, { row: 1, col: 4 })).toBe(
				true
			);
		});
	...
	describe('Soldier moves', () => {
		test('red soldier should move forward before crossing river', () => {
			...
			expect(isValidMove(board, { row: 6, col: 4 }, { row: 5, col: 4 })).toBe(
				true
			);
			expect(isValidMove(board, { row: 6, col: 4 }, { row: 6, col: 3 })).toBe(
				false
			); // No sideways
		});
		...
		test('red soldier should move forward and sideways after crossing river', () => {
			...
			expect(isValidMove(board, { row: 4, col: 4 }, { row: 3, col: 4 })).toBe(
				true
			);
			expect(isValidMove(board, { row: 4, col: 4 }, { row: 4, col: 3 })).toBe(
				true
			);
			expect(isValidMove(board, { row: 4, col: 4 }, { row: 4, col: 5 })).toBe(
				true
			);
			expect(isValidMove(board, { row: 4, col: 4 }, { row: 5, col: 4 })).toBe(
				false
			); // No backward
		});
		...
		test('black soldier should move forward before crossing river', () => {
			...
			expect(isValidMove(board, { row: 3, col: 4 }, { row: 4, col: 4 })).toBe(
				true
			);
			expect(isValidMove(board, { row: 3, col: 4 }, { row: 3, col: 3 })).toBe(
				false
			);
		});
		...
		test('black soldier should move forward and sideways after crossing river', () => {
			...
			expect(isValidMove(board, { row: 5, col: 4 }, { row: 6, col: 4 })).toBe(
				true
			);
			expect(isValidMove(board, { row: 5, col: 4 }, { row: 5, col: 3 })).toBe(
				true
			);
			expect(isValidMove(board, { row: 5, col: 4 }, { row: 5, col: 5 })).toBe(
				true
			);
			expect(isValidMove(board, { row: 5, col: 4 }, { row: 4, col: 4 })).toBe(
				false
			);
		});
	});
