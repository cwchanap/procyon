            ...
            for (let col = 0; col < SHOGI_BOARD_SIZE; col++) {
                ...
                if (piece && piece.color === currentPlayer) {
                    ...
                    for (const toPos of possibleMoves) {
                        ...
                        const canPromote = this.canPiecePromote(
                            piece,
                            fromPos,
                            toPos
                        );
...
Your move:`;
    ...
    createVisualBoard(gameState: ShogiGameState): string {
        ...
        for (let rank = 0; rank < SHOGI_BOARD_SIZE; rank++) {
            ...
            for (let file = 0; file < SHOGI_BOARD_SIZE; file++) {
                ...
                if (piece) {
                    ...
                    const rotated =
                        piece.color === 'gote' ? `v${symbol}` : ` ${symbol}`;
                    visual += rotated;
                } else {
                    visual += ' . ';
                }
        ...
        visual +=
            '        Promotion zone: Sente (ranks a-c), Gote (ranks g-i)\n';
    ...
    private getPossibleMovesForPiece(
        ...
        // Simplified move generation - in a real implementation,
        // you would import proper move generation from shogi library
        const moves: ShogiPosition[] = [];

        // Basic pawn movement as example
        if (piece.type === 'pawn') {
            ...
            if (
                ...
                !board[targetRow][position.col]
            ) {
                moves.push({ row: targetRow, col: position.col });
            }
        }
    ...
    private canPiecePromote(
        ...
        return (
            promotionZone.includes(from.row) || promotionZone.includes(to.row)
        );
    }
