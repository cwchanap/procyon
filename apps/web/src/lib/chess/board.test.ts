...
describe('Chess Board Utilities', () => {
	...
	describe('setPieceAt', () => {
		...
		test('should not crash on invalid position', () => {
			const board = createInitialBoard();
			const piece: ChessPiece = { type: 'queen', color: 'white' };
			expect(() => setPieceAt(board, { row: -1, col: 0 }, piece)).not.toThrow();
		});
	});
	...
	describe('isSquareOccupiedByAlly', () => {
		test('should return true when ally piece present', () => {
			const board = createInitialBoard();
			expect(isSquareOccupiedByAlly(board, { row: 0, col: 0 }, 'black')).toBe(
				true
			);
			expect(isSquareOccupiedByAlly(board, { row: 7, col: 0 }, 'white')).toBe(
				true
			);
		});
		...
		test('should return false when opponent piece present', () => {
			const board = createInitialBoard();
			expect(isSquareOccupiedByAlly(board, { row: 0, col: 0 }, 'white')).toBe(
				false
			);
		});
		...
		test('should return false for empty square', () => {
			const board = createInitialBoard();
			expect(isSquareOccupiedByAlly(board, { row: 3, col: 3 }, 'white')).toBe(
				false
			);
		});
	});
