...
- **Deviation from original spec (resolved):** The plan (Task 2 interface) documented `usePlayHistory` as returning `UsePlayHistoryReturn { savePlayHistory }`. The implemented hook returns `void` and keeps `savePlayHistory` as an internal `useCallback` invoked by the auto-save `useEffect`. No caller uses the return value — all four game components invoke `usePlayHistory({...})` as a statement, and the barrel (`hooks/index.ts`) exports only `usePlayHistory` + `UsePlayHistoryOptions` (not `UsePlayHistoryReturn`). Returning `savePlayHistory` would be dead code (YAGNI), so the return type was dropped rather than restored.
